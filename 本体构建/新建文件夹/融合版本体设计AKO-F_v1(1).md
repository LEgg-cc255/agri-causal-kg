...
**设计定位**：在保持"事件是知识单元载体、不是图谱节点"定位不变的前提下，在知识单元层定义事件间关联属性，显式表达事件知识单元之间的时序、因果与抑制关系。事件间关联仅作用于 `ku_type=EVENT` 的知识单元，不改变实体层、关系层、事件层及七元组的核心结构，不影响五维约束算子与多模态融合的运作。

**三类事件间关联属性**：

| 属性 | domain → range | 语义 | 生成方式 |
|---|---|---|---|
| `ku_precedes`（先于） | KnowledgeUnit → KnowledgeUnit | 本事件在时间上先于另一个事件 | 通过 TimeTriple 比较完全自动生成 |
| `ku_causes`（导致/触发） | KnowledgeUnit → KnowledgeUnit | 本事件导致或触发了另一个事件（如环境影响→触发→病害发生） | 基于因果规则库+共享实体半自动生成，人工审核 |
| `ku_inhibits`（抑制） | KnowledgeUnit → KnowledgeUnit | 本事件抑制了另一个事件的发生或扩散（如防治→抑制→扩散） | 基于防治事件+扩散事件+共享病害半自动生成 |

**事件链示例**：

```
ku_env_highhumidity (环境影响, 高温高湿, 6月华南)
  ─ku_causes→ ku_event_blast_202606 (病害发生, 稻瘟病, 6月华南)
    ─ku_precedes→ ku_control_tricyclazole (防治, 三环唑, 7月华南)
      ─ku_inhibits→ ku_spread_blast (扩散, 稻瘟病, 7-8月长江流域)
```

**与核心结构的关系**：
- 实体层 10 类、关系层 10 类核心关系（实体→实体）、事件层 8 类、七元组结构——全部不变；
- 事件间关联属于知识单元层的附加属性，不进入实体关系层，不进入 M_type 10×10 实体兼容性 Mask；
- 五维约束算子仍然作用于七元组，事件间关联可作为因果合理性校验的额外输入；
- 多模态融合仍然是槽位级对齐，事件间关联是融合之后的知识单元关联。

**生成成本**：`ku_precedes` 100% 自动生成（零成本）；`ku_causes`/`ku_inhibits` 自动候选+人工审核，预计每条事件链审核时间 <1 分钟，80 篇文本预计产生 30-60 条事件间关系，人工审核量约 30-60 分钟，不显著增加标注成本。

**对研究的增益**：
1. 支撑事件链推理：可直接查询"稻瘟病发生之前发生了什么""什么事件导致了稻瘟病""防治事件抑制了什么"；
2. 增强因果合理性校验：事件间因果关系可与因果规则库交叉验证（如 ku_causes 声明但规则库无对应规则→标记待验证）；
3. 丰富 GraphRAG 下游能力：事件链检索、过程式问答（"稻瘟病从发生到防治的完整过程"）；
4. 可作为创新点1（约束驱动可信推理）的扩展应用：基于五维约束校验后的可信知识单元，进一步构建事件间关联网络。
